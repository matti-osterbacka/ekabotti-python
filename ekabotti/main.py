...
async def fetch_first_message(test, channel):
    dt = datetime.combine(date=date.today(), time=time.hour(3)).astimezone(tz=timezone(timedelta(hours=2), 'Europe/Helsinki'))
    print(dt)
    if test:
        dt = datetime.combine(date=date.fromisoformat("2024-06-18"), time=time.hour(3)).astimezone(tz=timezone(timedelta(hours=2), 'Europe/Helsinki'))
    first_message = None
